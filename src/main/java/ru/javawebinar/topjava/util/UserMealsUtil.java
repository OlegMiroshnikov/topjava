...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        Map<LocalDate, List<UserMeal>> mapMealsByDay = new HashMap<> ();
        for (UserMeal userMeal : meals) {
            LocalDate mealDate = userMeal.getDateTime ().toLocalDate ();
            mapCaloriesByDay.merge (mealDate, userMeal.getCalories (), Integer::sum);
            if (TimeUtil.isBetweenInclusive (userMeal.getDateTime ().toLocalTime (), startTime, endTime)) {
                mapMealsByDay.merge (mealDate, new ArrayList<> (Arrays.asList (userMeal)),
                        (v1, v2) -> {
                            v1.addAll (v2);
                            return v1;
                        });
        ...
        mapMealsByDay.forEach ((k, v) -> {
                    v.forEach (m -> mealsTo.add (new UserMealWithExcess (m.getDateTime (), m.getDescription (), m.getCalories (),
                            mapCaloriesByDay.get (m.getDateTime ().toLocalDate ()) > caloriesPerDay)));
                }
        );
        return mealsTo;
    }
    ...
    public static List<UserMealWithExcess> filteredByStreams(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate, Integer> mapCaloriesByDay = meals.stream ()
