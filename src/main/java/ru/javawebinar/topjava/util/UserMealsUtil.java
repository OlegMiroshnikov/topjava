...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        meals.forEach((m) -> mapCaloriesByDay.merge(m.getDateTime().toLocalDate(), m.getCalories(), Integer::sum));
        meals.forEach((m) -> {
            if (TimeUtil.isBetweenInclusive(m.getDateTime().toLocalTime(), startTime, endTime)) {
                mealsTo.add(new UserMealWithExcess(m.getDateTime(), m.getDescription(), m.getCalories(),
